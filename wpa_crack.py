...
def parse_targets(filename):
    ...
    try:
        ...
        with open(filename, 'r') as csvfile:
            ...
            for row in targetreader:
                ...
                if not hit_clients:
                    ...
                    if bssid: targets[bssid] = [ssid,row[3].strip(),enc,power,wps,0]
                else:
                    if len(row) < 6:
                        continue
        ...
        targets=sorted(targets.items(),key=lambda item:item[1][3], reverse=True)
    except IOError as e:
...
class Wpa_Attack:
    ...
    def ConfirmRunningAsRoot(self):
        if os.getuid() != 0:
            print(R+' [!]'+O+' ERROR:'+G+'wpa_crack'+O+' must be run as '+R+'root'+W)
            print(R+' [!]'+O+' login as root ('+W+'su root'+O+') or try '+W+'sudo ./wpa_crack.py'+W)
            exit(1)
    ...
    def get_iface(self):
        ...
        if len(ifaces)>1:
            print(GR + " [+] " + W + "Found "+G+str(len(ifaces))+W+" wireless devices...")
        ...
        try:
            while True:
                ...
                print('\n %s %s wireless networks. %s target%s found...\r' % (
                    ...
                    G + str(len(self.targets)) + W, '' if len(self.targets) == 1 else 's',),end='')
        except KeyboardInterrupt:
            pass
        print('')
        send_interrupt(proc)
